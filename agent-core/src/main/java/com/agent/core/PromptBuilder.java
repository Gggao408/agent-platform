...
public class PromptBuilder {
    ...
    public String build(List<ToolDefinition> tools) {
       return"""
               ...
               重要规则：
               1. 当用户问你任何关于文档、知识库、上传内容的问题时，必须先调用 search_knowledge 工具查找，不要直接回答
               2. 需要查数据库时调用 query_database，需要发HTTP请求时调用 http_get/http_post
               3. 工具返回结果后，基于结果继续思考和回答
               4. 信息足够时直接给出答案，不要再调用工具
               5. 如果工具调用失败，尝试其他方式或如实告知用户
               6. 不要编造数据——所有事实性信息必须来自工具返回结果
               7. 所有回答使用中文，回答要简洁、准确、有帮助
               """;
    }
}
